...
public abstract class ChatListFragment extends BaseFragment implements IFChatListAction {
    ...
    public void refreshChatList(boolean isFirst) {
        ...
        apiService.getChatList(data).compose(RxUtils.rxSchedulerHelper())
            ...
            public void accept(ChatListBean chatListBean) throws Exception {
                getActivity().runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        showChatList(chatListBean.getListDataInfo((String) SPManager.getInstance(getContext()).get(SPManager.SP_KEY_USER_ID, ""), false));
                    }
                });
            }
...
//    }
    ...
    @Subscribe(threadMode = ThreadMode.MAIN)
    public void handleDefaultEvent(ConversationListDataInfo daoChatListInfo) {
        refreshChatList(false);
    }
}
