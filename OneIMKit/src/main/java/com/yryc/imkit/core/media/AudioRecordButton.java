...
public class AudioRecordButton extends AppCompatTextView implements AudioManager.AudioStageListener {
    ...
    public AudioRecordButton(Context context, AttributeSet attrs) {
        ...
        //获取录音保存位置
        String dir = getAppRecordDir().toString();
        //实例化录音核心类
        mAudioManager = AudioManager.getInstance(dir);
        mAudioManager.setOnAudioStageListener(this);
        DISTANCE_Y_CANCEL = PixelUtils.dip2px(getContext(), 200);
            ...
            public boolean onLongClick(View v) {
                ...
                permissionHelper.requestPermissions(getResources().getString(R.string.tip_permission), new PermissionHelper.PermissionListener() {
                    ...
                    public void doAfterDenied(String... permission) {
                        Toast.makeText(getContext(), "请授权,否则无法录音", Toast.LENGTH_SHORT).show();
                    }
        ...
        if (mCurrentState != state) {
            ...
            switch (mCurrentState) {
                ...
                case STATE_RECORDING:
                    setBackground(getResources().getDrawable(R.drawable.shape_stroke_gray_6_solid_gray_8_radius_5));
                    setText(R.string.hang_up_finish);//松开结束
                    if (isRecording) {
                        // 复写dialog.recording();
    ...
    public File getAppRecordDir() {
        File recordDir = new File(Config.Dir.VOICE);
        if (!recordDir.exists()) {
            recordDir.mkdir();
        }
        return recordDir;
